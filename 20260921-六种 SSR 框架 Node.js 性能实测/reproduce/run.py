"""Run only resources labelled ssrbench.run=20260921. Requires prepared Docker volume.
...
def stop_app():
    if docker('ps','-aq','--filter','name=^ssrbench-app$'):
        ...
        assert label=='20260921'
        docker('rm','-f','ssrbench-app')
def start_app(name):
    stop_app()
    docker('run','-d','--name','ssrbench-app','--label',LABEL,'--network','ssrbench-20260921','--cpuset-cpus=0-1','--cpus=2','--memory=1536m','--memory-swap=1536m','-v','ssrbench-20260921:/bench','-w',f'/bench/apps/{name}','-e','NODE_ENV=production','-e','NODE_OPTIONS=--require=/bench/metrics.cjs --max-old-space-size=1024','-e','BENCH_METRICS_PORT=9100','-e','NEXT_TELEMETRY_DISABLED=1','-e','HOST=0.0.0.0','-e','HOSTNAME=0.0.0.0','-e','PORT=3000',IMAGE,*COMMANDS[name])
    for i in range(30):
