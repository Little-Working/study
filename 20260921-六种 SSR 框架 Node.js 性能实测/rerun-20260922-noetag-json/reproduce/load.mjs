...
const stats=a=>{if(!a.length)return {count:0};a.sort((a,b)=>a-b);const q=p=>a[Math.min(a.length-1,Math.ceil(p*a.length)-1)];return {count:a.length,mean:a.reduce((s,v)=>s+v,0)/a.length,min:a[0],max:a.at(-1),p50:q(.5),p95:q(.95),p99:q(.99)};};
if(!isMainThread){
 ...
 async function request(){
  ...
  return new Promise(resolve=>{
   ...
   const req=http.get(`http://ssrbench2-app:3000/?size=${size}&delay=${delay}&rid=${rid}`,{agent,headers:{'accept':'text/html','accept-encoding':'identity','user-agent':'ssrbench-v2/1.0'}},res=>{
 ...
 if(config.phase==='warmup'&&config.seconds===8&&readdirSync('/bench/results').filter(f=>f.startsWith('matrix-')&&f.endsWith('.json')).length===324)config.seconds=30;
 ...
 if(measure){await get('http://ssrbench2-app:9100/reset');await get('http://ssrbench2-backend:9100/reset');await get('http://ssrbench2-backend:4000/reset');}
 ...
 if(measure){app=await get('http://ssrbench2-app:9100/');backend=await get('http://ssrbench2-backend:9100/');backendRequests=await get('http://ssrbench2-backend:4000/stats');}
