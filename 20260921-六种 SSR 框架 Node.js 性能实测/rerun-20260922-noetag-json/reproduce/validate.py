from common import *
for f in ['metrics.cjs','audit.cjs','load.mjs','__tests__']:docker('cp',str(HERE/f),'ssrbench2-builder:/bench/'+f)
(OUT/'sentinel-selftest.json').write_text(docker('exec','ssrbench2-builder','node','/bench/__tests__/audit.test.cjs'))
calibration={}
try:
 for name in FRAMEWORKS:
  start_app(name,audit=True)
  result=docker('exec','ssrbench2-load','node','/bench/__tests__/page.test.mjs',f'/bench/results/{name}-validation.json')
  print(name,result,flush=True);copy_result(name+'-validation.json')
  (OUT/(name+'-validation-runtime.log')).write_text(docker('logs','ssrbench2-app'))
