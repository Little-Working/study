...
LABEL='ssrbench.run=20260924-runtime';RESOURCE='ssrbench3-20260924'
...
def stop_app():
 if docker('ps','-aq','--filter','name=^ssrbench3-app$'):
  assert docker('inspect','-f','{{index .Config.Labels "ssrbench.run"}}','ssrbench3-app')=='20260924-runtime'
  docker('rm','-f','ssrbench3-app')
def start_app(name,mode='lean',audit=False):
 ...
 docker('run','-d','--name','ssrbench3-app','--label',LABEL,'--network',RESOURCE,'--cpuset-cpus=0-1','--cpus=2','--memory=1536m','--memory-swap=1536m','-v',RESOURCE+':/bench','-w',f'/bench/apps/{name}','-e','NODE_ENV=production','-e','NODE_OPTIONS='+opts,'-e','BENCH_METRICS_MODE='+mode,'-e','NEXT_TELEMETRY_DISABLED=1','-e','HOST=0.0.0.0','-e','HOSTNAME=0.0.0.0','-e','PORT=3000',IMAGE,*COMMANDS[name])
 for _ in range(60):
  if docker('inspect','-f','{{.State.Running}}','ssrbench3-app')!='true':raise RuntimeError(docker('logs','ssrbench3-app'))
  p=subprocess.run(['docker','exec','ssrbench3-load','node','-e',"fetch('http://ssrbench3-app:3000/?rid=ready').then(async r=>{if(r.status!==200||!(await r.text()).includes('BENCH_END'))process.exit(1)}).catch(()=>process.exit(1))"],capture_output=True)
  if p.returncode==0:return
  time.sleep(.5)
 raise RuntimeError('Startup timeout: '+name)
def copy_result(name):
 docker('cp','ssrbench3-load:/bench/results/'+name,str(OUT/name))
def cleanup():
 removed={}
