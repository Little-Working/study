...
const fixtures=await get('http://ssrbench3-backend:4000/fixtures');
for(const size of ['hello','medium','large']){
 const r=await fetch(`http://ssrbench3-backend:4000/data?size=${size}&delay=50&rid=backend-validation`);
...
await get('http://ssrbench3-backend:4000/reset');
for(const size of ['hello','medium','large'])for(const delay of [50,200,500])for(const encoding of ['identity','gzip, deflate, br'])for(let repeat=0;repeat<2;repeat++){
 ...
 const r=await fetch(`http://ssrbench3-app:3000/?size=${size}&delay=${delay}&rid=${rid}`,{headers:{'accept-encoding':encoding,'accept':'text/html','if-none-match':'"nonmatching-test-value"'}});
...
const stats=await get('http://ssrbench3-backend:4000/stats'),metrics=await get('http://ssrbench3-app:9100/');
